...
llm1=("-m /ai/models/Ministral-3-8B-Instruct-2512-Q8_0.gguf -ngl 130  --top-k 20 --min-p 0.1 --top-p 0.95 --device CUDA1 --repeat-penalty 1.4 --repeat-last-n 2 --predict 22512 --reasoning-format none --cache-type-k q8_0 --jinja -fa auto"
      "-m /ai/models/Ministral-3-14B-Instruct-2512-Q8_0.gguf -ngl 130  --top-k 20 --min-p 0.1 --top-p 0.95 --device CUDA1 --repeat-penalty 1.4 --repeat-last-n 2 --predict 22512 --reasoning-format none --cache-type-k q8_0 --jinja -fa auto"
...
#    "-m /ai/models/Nemotron-3-Nano-30B-A3B-UD-Q4_K_XL.gguf -ngl 130 -np 1-cb --top-k 20 --min-p 0.1 --top-p 0.95 --device CUDA1 --repeat-penalty 1.8 --repeat-last-n 3 --predict 22512 --no-mmap -fa auto --numa distribute --reasoning-format none --cache-type-k q8_0 --jinja --reasoning-budget 0"
#    "-m /ai/models/GigaChat-20B-A3B-instruct-v1.5-q6_K.gguf  -ngl 130 -np 1-cb --top-k 20 --min-p 0.1 --top-p 0.95 --device CUDA1 --repeat-penalty 1.8 --repeat-last-n 3 --predict 22512 --no-mmap  --numa distribute --reasoning-format none --cache-type-k q8_0 --jinja -fa auto --reasoning-budget 0")
...
llm2=("-m /ai/models/HY-MT1.5-7B-Q8_0.gguf -ngl 130   --top-k 20 --min-p 0.1 --top-p 0.6 --repeat-penalty 1.4 --device CUDA0 --predict 22512 -fa auto"
	"-m /ai/models/HY-MT1.5-7B-Q8_0.gguf -ngl 130   --top-k 20 --min-p 0.1 --top-p 0.9 --repeat-penalty 1.4 --device CUDA0 --predict 22512 -fa auto"
...
for i in "${!temp1[@]}"; do
    ...
    echo " Run main test"\n
    # Запуск основного ПО
    TEMP2=$temp2 TEMP=$temp1 SHORT=$short FAST_TRANS=0 CONCURRENT_LIMIT=1 /root/miniconda3/envs/ttsv2/bin/python app.py > cargo_$short-$temp1-$temp2.log 2>&1 &
    app_pid=$!

    # Ожидание завершения работы основного ПО
    wait $app_pid
    
    TEMP2=$temp2 TEMP=$temp1 SHORT=$short FAST_TRANS=0 CONCURRENT_LIMIT=3 /root/miniconda3/envs/ttsv2/bin/python app.py > carg_con_$short-$temp1-$temp2.log 2>&1 &
    app_pid2=$!

    wait $app_pid2

        
    TEMP2=$temp2 TEMP=$temp1 SHORT=$short FAST_TRANS=0 MODEL_TRANSLATE="Hunyuan" /root/miniconda3/envs/ttsv2/bin/python app.py > carg_HY_$short-$temp1-$temp2.log 2>&1 &
    app_pid2=$!
