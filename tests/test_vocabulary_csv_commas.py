...
def create_test_dictionary_with_commas(temp_dir):
    ...
    # Write header and entries using the real dictionary format:
    ...
    with open(dict_file, 'w', encoding='utf-8') as f:
        ...
        for entry in test_entries:
            ...
            f.write(f'{entry["source"]} = {buf.getvalue().rstrip()}\n')
...
def test_csv_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Save using the REAL dictionary format:
        #   Format: source = target, category, gender, notes
...
def main():
    ...
    try:
        ...
        print("\nFAIL: test_csv_loading failed.")
        return False

    try:
        test_llm_json_response_parsing()
    except AssertionError:
        print("\nFAIL: test_llm_json_response_parsing failed.")
        return False

    try:
        test_csv_roundtrip()
    except AssertionError:
        print("\nFAIL: test_csv_roundtrip failed.")
        return False
